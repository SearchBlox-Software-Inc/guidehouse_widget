...
import {
  Collapse, Button, Label, Input, DropdownToggle,
  DropdownMenu,
  DropdownItem,
                  ...
                  <div className="facetEachDiv">
                    {
                      ...
                      <div className="facetSubDiv" >
                        <span className="facetHeading">{facet.display}
                          {/* <i className="upArrow"/> */}
                        </span>
                      </div>
                    }
                      ...
                      <ul style={{ listStyleType: 'none' }}>
                        {
                          facet[field].map((filter, index) => {
                            if (!filter.rangeField) {
                              return <li key={index} className={(this.state[field] < index + 1) ? "facetValue showClass" : "facetValue showClass"} onClick={(e) => this.toggleFilter(e, field, filter.filterName)}>
                                <div className={`facetChild ${filter.filterSelect ? 'activeFilter' : 'inactiveFilter'} ${facet.facetField === "contenttype" ? "contentType" : "notcontentType"}`} title={filter.filterName}>
                                  <p className="mb-0 d-flex">
                                    <span
                                    // className={[filter.filterSelect ?'activefacet-count':'inactivefacet-count'].join(" ")}
                                    >{filter.count}</span>
                                  </p>
                                  <p className="">
                                    {/* <label className="checkboxContainer">*/}
                                    {/*<input type="checkbox" defaultChecked={filter.filterSelect} style={{marginRight:"5px",cursor:"pointer"}}/>*/}{' '}
                                    {/* <a href=""  title={filter.filterName}
                                ><span className="checkmark"/></a> */}
                                    {/*</label> */}
                                    {filter.filterName.replace(/&quot;/g, '"').replace(/&amp;/g, "&")}
                                    {/* {filter.filterName.length <=5 ? <span className="customtext" aria-hidden="true">facet</span>
                              :""
                              } */}
                                  </p>
                                  <p>
                                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      <line y1="-0.5" x2="11.4492" y2="-0.5" transform="matrix(0.707044 0.707169 -0.707044 0.707169 0.843506 1.45166)" stroke="#1B1B1B" />
                                      <line y1="-0.5" x2="11.4492" y2="-0.5" transform="matrix(-0.707044 0.707169 0.707044 0.707169 9.15649 1.45166)" stroke="#1B1B1B" />
                                    </svg>
